...
def process(case):
    ...
    multipleCheques = (Features(lines=re.findall("\. ((?:[A-Z][a-z0-9]|\([a-z]+\)| \d+\. ).{,100}?(?:[Ss]ection 219|[sS]ection 220|same (?:person|parties)|same transaction|all the cheques|cheques|another cheque).{,100}?\.) ", order.text, flags=re.DOTALL), path=order.path, type=order.type) for order in case.interimOrders+case.finalOrders)
    case.multipleCheques = [a for a in multipleCheques if len(a.lines) > 0]

    summons = (Features(lines=re.findall("\. ((?:[A-Z][a-z0-9]|\([a-z]+\)| \d+\. ).{,100}?(?:[Ss]ection 143|summarily|[Ss]ection 262|[Ss]ection 265|not exceeding|day.to.day|reasoned order|speaking order|\W[dcp]w\W|witness).{,100}?\.) ", order.text, flags=re.DOTALL), path=order.path, type=order.type) for order in case.interimOrders+case.finalOrders)
    case.summons = [a for a in summons if len(a.lines) > 0]

    outcome = (Features(lines=re.findall("\. ((?:[A-Z][a-z0-9]|\([a-z]+\)| \d+\. ).{,100}?(?:not guilty|acquitted|convicted|guilty|sentenced).{,100}?\.) ", order.text[-1000:], flags=re.DOTALL), path=order.path, type=order.type) for order in case.finalOrders+case.interimOrders)
    case.outcome = [a for a in outcome if len(a.lines) > 0]
    
    return case

# iterate through case data and orders
def caseCrunch(j, orderData, pdfs):
    ...
    ocombine = []
    for o in omatch:
        r = [i for i in rmatch if i['PDFFileName']==o.path][0]
        x = OrderDetails(path=o.path, text=o.text, lang=o.lang, cnr=o.cnr, number=r['OrderNumber'], date=r['OrderDate'], details=r['OrderDetails'], type=r['OrderType'])
        ocombine.append(x)
    case = Case(actName = str(j['UnderActs']),
                ...
                transfer = [Transfer(date=i["TransferDate"], fromJudge=i["FromCourtJudge"], toJudge=i["ToCourtJudge"]) for i in j["transfer"] if i],
                uid = j['Id'],
                year = j['Year'],
...
def main():
    ...
    if not os.path.isfile("../DATA/caseTransJoin.pickle"):
        ...
        print("Read transfer data")
        transferData = pd.concat([pd.read_csv(transCSV,
                                          usecols=['Id', 'CaseInformationId', 'TransferDate',
                                                   'FromCourtJudge', 'ToCourtJudge'],
                                          parse_dates=False) for transCSV in tlist], ignore_index=True)
    
        transferData.rename(columns={"Id":"trId"}, inplace=True)
        tdata = transferData.to_dict(orient="records")
        jdata = caseData.to_dict(orient="records")
        #join transfer data and caseData
        print("Join case and transfer data")
        for j in tqdm(jdata):
            j["transfer"] = [(i["TransferDate"],i["FromCourtJudge"],i["ToCourtJudge"]) for i in tdata if i["CaseInformationId"]==j["Id"]]
        pickle.dump(file=open("../DATA/caseTransJoin.pickle","wb"), obj=jdata)
    else:
        print("Found joined transfer, case data")
        jdata = pickle.load(file=open("../DATA/caseTransJoin.pickle","rb"))
    ...
    if os.path.isfile("../DATA/sample4.pickle"):
        print("Found pre-processed cases")
        cases = pickle.load(open("../DATA/sample4.pickle","rb"))
    else:
        print(datetime.datetime.now())
        partCaseCrunch = functools.partial(caseCrunch, orderData=orderData, pdfs=pdfs)
        with ProcessPoolExecutor(max_workers=16) as exr:
            # empty list to hold case data
            crunch = list(tqdm(exr.map(partCaseCrunch, jdata, chunksize=8000), total=len(jdata)))
        cases = [s.__dict__ for s in tqdm(crunch)]
        pickle.dump(file=open("../DATA/sample4.pickle", "wb"), obj=cases)
        print(datetime.datetime.now())
    ...
    # feat = [e for e in eng if len(e["absconding"] + e["award"] + e["amount"] + e["outcome"] + e["judgeChange"] + e["jurisdiction"] + e["mediation"] + e["plausibleDefence"] + e["multipleCheques"] + e["summons"]) > 0]
    ...
    # judgeChange = [ntDict(e) for e in feat if len(e["judgeChange"])>0]
    # json.dump(fp=open("../DATA/MANUAL_CHECK/judgeChange.json","w"), obj=judgeChange)
    ...
    json.dump(fp=open("../DATA/sample4.json", "w"), obj=[ntDict(c) for c in cases])
